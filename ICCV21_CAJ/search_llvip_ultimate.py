...
def get_feature_layer2(model, img_tensor, mode='visible'):
    with torch.no_grad():
        ...
        x = model.base_resnet.base.layer2(x) # Native Stride = 8
    return x
...
def main():
    ...
    RESULT_DIR = 'search_results_ultimate'
    os.makedirs(RESULT_DIR, exist_ok=True)
    
    # 设定上采样倍率为 4 (Stride 8 -> 2)
    UPSAMPLE_SCALE = 4.0 
    ...
    all_ious = []
    print(f"开始最终极限实验 (Layer 2 + Upsample x{UPSAMPLE_SCALE})...")
